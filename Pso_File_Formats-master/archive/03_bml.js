...
const FILENAME = "bm_ene_lappy_ap.bml";
...
function extract_bml(filename){
  ...
  for(i = 0; i < num_items; i++){
    ...
    if(entry[i].csize % 0x10)
      entry[i].csize += (0x10 - (entry[i].csize % 0x10));
    if(entry[i].pvm_csize % 0x10)
      entry[i].pvm_csize += (0x10 - (entry[i].pvm_csize % 0x10));

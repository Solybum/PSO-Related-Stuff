...
function export_xvm(file, array){
  ...
  if(array && array.length != num_elements){
    throw new Error("Invalid array parameter provided");
  }
  ...
  for(var i = 0; i < num_elements; i++){
    ...
    var data = new Buffer(header.data_length + 0x40);
    buffer.copy(data, 0, offset, offset + header.data_length + 0x40);
    offset += data.length;

    //write_file(header, data);
    fs.writeFileSync("output/test.pvr", data);
    break;
  }

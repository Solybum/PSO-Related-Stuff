...
module.exports = function(filename){
  ...
  this.readInt32 = function(){
    var integer = buffer.readInt32LE(fp);
    fp += 4;
    return integer;
  }
  ...
  this.search = function(val){
    var found = false;
    for(var pos = fp; pos < buffer.length - 4; pos += 0x04){
      if(buffer.readUInt32LE(pos) == val){
        return pos;
      }
    }
    return -1;
  ...
  this.extract = function(start, end){
    var len = end - start;
    var buff = new Buffer(len);
    buffer.copy(buff, 0, start, end);
    return buff;
  }

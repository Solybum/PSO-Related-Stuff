...
for(var i = 0; i < num_section; i++){
  var start = fp.getPosition();
  map_section.push({
    unknown1 : fp.readDword() + 16,
    pos : {
      x : fp.readSingle(),
      y : fp.readSingle(),
      z : fp.readSingle()
    ...
    single : fp.extract(start, start + 16)
  });
}
...
start += dword_to_hex(data_str.length/2);
...
for(var i = 0; i < models.length; i++){
  ...
  model_str += dword_to_hex(13 * 0x04 + node.model.tBuffer.length + node.model.vBuffer.length);
  model_str += node.buffer_str.toString("hex");
  model_str += dword_to_hex(0); //child
  model_str += dword_to_hex(0); //sibling

  model_str += node.model.tBuffer.toString("hex");
  model_str += node.model.vBuffer.toString("hex");

  model_str += dword_to_hex(13 * 0x4 + node.model.tBuffer.length); //vector
  model_str += dword_to_hex(13 * 0x4); //texture
  model_str += node.center.toString("hex");
  //console.log(node.center.toString("hex"));
  ...
  //model_str = data_str + model_str; //NJTL
...
function str_byte_len(str){
  return new Buffer(str, "hex").length/2;
}

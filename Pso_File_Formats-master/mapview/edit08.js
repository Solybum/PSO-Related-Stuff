  ...
  for(var i = 0; i < areas; i++){
    ...
    for(var k = 0; k < MapSection[i].VertexA_Count; k++){
      ...
      for(var j = 0; j < ic; j++){
        ...
        fp += 4;
        e = buffer.readUInt32LE(fp);  //indices
        fp += 4;
        f = buffer.readUInt32LE(fp); //num of indices
        fp += 4;
...
function void_WriteOutput(filename){
  ...
  out_material = util.format("%s/texture.mtl",out_dir);
  matf = fs.openSync(out_material, "w");
  fs.writeSync(modf, 'mtllib texture.mtl\r\n');
  fs.writeSync(modf, 'g '+base+'\r\n');
  ...
  for(var i = 0; i < MapSectionData.length; i++){
    ...
    for(var k = 0; k < vlist.length; k++){
      ...
      //fs.writeSync(modf, 'usemtl mat'+block+'\r\n');
      ...
      for(j = 0; j < vlist[k].indice.length; j++){
        ...
        fs.writeSync(matf, 'newmtl mat'+block+'\r\n');
        fs.writeSync(matf, 'illum 4\r\n');
        fs.writeSync(matf, 'Ld 0.5 0.5 0.5\r\n');
        fs.writeSync(matf, 'Ka 0.0 0.0 0.0\r\n');
        fs.writeSync(matf, 'Tf 0.3 0.3 0.3\r\n');
        fs.writeSync(matf, 'map_Kd img/Image '+tnum+'.dds\r\n');
        ...
        for(var n = 0; n < indice_list.length - 1; n++){
          ...
          if(!n % 2){
            str = util.format("f %d/%d/%d %d/%d/%d %d/%d/%d\r\n", a,a,a, b,b,b, c,c,c);
            fs.writeSync(modf, str);
            str = util.format("f %d/%d/%d %d/%d/%d %d/%d/%d\r\n", a,a,a, c,c,c, b,b,b);
            fs.writeSync(modf, str);
          }else{

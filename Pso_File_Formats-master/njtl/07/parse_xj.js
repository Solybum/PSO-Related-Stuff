...
function parse_xj(filename){
  ...
  var nodes = [];
  parse_node(0);
  var str = JSON.stringify(nodes, null, "\t");
  fs.writeFileSync("ohgodno.json", str);
  ...
  **/
  function eval_flags(uint){
    ...
    var flag = {
      skip_pos : Boolean(uint & 1),
      skip_ang : Boolean(uint & 2),
      bi_str : uint.toString(2)
    };

    while(flag.bi_str.length < 8){
      flag.bi_str = "0" + flag.bi_str
    }
...
function write_obj_file(nodes){
  ...
  for(var i = 0; i < nodes.length; i++){
    ...
    n = nodes[i];
    n.p = n.p || {id:null,flags:{}};
    n.s = n.s || {id:null,flags:{}};
    ...
    if(n.s.id){
      ...
      console.log(n.id + " " + n.s.id);
      n.position[0] += n.s.position[0];
      n.position[1] += n.s.position[1];
      n.position[2] += n.s.position[2];

      /*
      n.apply_pos[0] = n.position[0];
      n.apply_pos[1] = n.position[1];
      n.apply_pos[2] = n.position[2];
      */
    }else if(n.p.id){
      ...
      n.position[0] += n.p.position[0];
      n.position[1] += n.p.position[1];
      n.position[2] += n.p.position[2];
    ...
    var degrees = {
      x : parseInt(n.angle[0] / 0xFFFF * 360),
      y : parseInt(n.angle[1] / 0xFFFF * 360),
      z : parseInt(n.angle[2] / 0xFFFF * 360)
    };

    var position = JSON.stringify({
      x : n.position[0].toFixed(6),
      y : n.position[1].toFixed(6),
      z : n.position[2].toFixed(6),
    }).replace(/"/g, "")
    .replace(/:/g, " : ")
    .replace(/,/g, ", ");

    var center = JSON.stringify({
      x : n.model.center[0].toFixed(6),
      y : n.model.center[1].toFixed(6),
      z : n.model.center[2].toFixed(6)
    }).replace(/"/g, "")
    .replace(/:/g, " : ")
    .replace(/,/g, ", ");

    var flags_str = n.flags.bi_str.replace(/(.{1})/g,"$1 ");
    delete n.flags.bi_str;
    console.log(n.id);
    console.log("Parent: %s", n.p.id);
    console.log("Sibling: %s", n.s.id);
    console.log("Position: ", position);
    console.log("Angle: ", degrees);
    console.log("Center: ", center);
    console.log("Radius: ", n.model.radius.toFixed(6));
    console.log("Eval Flags: ", flags_str);
    console.log(n.flags);
    console.log("");

...
module.exports = parse_njcm;

/*
 * parse_njtl
 */
function parse_njcm(fp, px){
  ...
  if(node.child){
    ...
    node.child = parse_njcm(fp, mx);
  }

  if(node.sibling){
    fp.seek_set(node.sibling);
    node.sibling = parse_njcm(fp, px);
  }

  if(!px){
    node.toString = toString;
    node.apply_rotate = apply_rotate;
    node.apply_translate = apply_translate;
  }
 ...
 * toString
 */
 function toString(len, rot, trans) {
 	var str = "";
 	var group_num = 1;
 	var a, b, c;
 	var face_str = "f %d/%d %d/%d %d/%d\r\n";
 	var v_length = len || 1;
  var rot_matrix = MatrixUtil.MakeRotate(rot);
 	make_string(this);
 	return { "str" : str, "len" : v_length };
    ...
    if(!node.model){
      ...
      var verts = MatrixUtil.Apply2(rot_matrix, vlist[i]);
      verts.x += trans.x;
      verts.y += trans.y;
      verts.z += trans.z;
 ...
 * rotate
 */
function apply_rotate(coord){
  var matrix = MatrixUtil.MakeRotate(coord);
  apply_to_verts(this);

  function apply_to_verts(node){
    for(var i = 0; i < node.model.vertex; i++){
      var verts = MatrixUtil.Apply2(matrix, node.model.vertex[i]);
      node.model.vertex[i].x = verts.x;
      node.model.vertex[i].y = verts.y;
      node.model.vertex[i].z = verts.z;
    }

    if(node.child){
      apply_to_verts(node.child);
    }

    if(node.sibling){
      apply_to_verts(node.sibling);
    }
  }
}
 ...
 * translate
 */
function apply_translate(coord){
  apply_to_verts(this);

  function apply_to_verts(node){
    for(var i = 0; i < node.model.vertex; i++){
      node.model.vertex[i].x += coords.x;
      node.model.vertex[i].y += coords.y;
      node.model.vertex[i].z += coords.z;
    }
